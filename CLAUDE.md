...
- **Minting an R2 API token requires the Cloudflare dashboard — wrangler can't.** `wrangler` has no `r2 token` subcommand, the dedicated `/accounts/{id}/r2/tokens` REST endpoint 404s, and the OAuth scopes wrangler holds don't include R2 admin. Mint via the dashboard (`R2 → Manage R2 API Tokens → Create token`), then `wrangler secret put R2_ACCESS_KEY_ID` / `R2_SECRET_ACCESS_KEY` from a user shell. Full procedure: [`docs/runbooks/r2-token-mint.md`](./docs/runbooks/r2-token-mint.md). Hit while wiring the first prod deploy (Day 5).
